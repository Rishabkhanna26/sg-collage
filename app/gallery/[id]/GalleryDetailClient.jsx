...
export function GalleryDetailClient({ section }) {
  ...
  const visibleYears = activeYear
    ? section.years.filter((y) => y.label === activeYear)
    : section.years;

  const lightboxImages =
    lightbox.yearIdx !== null
      ? section.years[lightbox.yearIdx].images
      : [];
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>{totalImages} photos across {section.years.length} academic years</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-14">
            {visibleYears.map((year, yearIdx) => {
              const realYearIdx = section.years.findIndex((y) => y.label === year.label);
              return (
                <YearGrid
                  key={year.label}
                  year={year}
                  onOpen={(imgIdx) => setLightbox({ yearIdx: realYearIdx, imgIdx })}
                />
              );
            })}
          </div>
